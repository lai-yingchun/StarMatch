...
export default function CelebrityFeaturePage() {
  ...
  useEffect(() => {
    ...
    const keepNames = new Set(["lulu黃路梓茵", "峮峮吳函峮"]);
    const removeNames = new Set(["郭雪芙", "許瑋甯", "謝欣穎"]);

    const labelPoints = tsneData.filter((d) => {
      // 若為指定去除名單 → 永不顯示
      if (removeNames.has(d.artist)) {
        return false;
      }
    ...
    jobs.forEach((job, i) => {
      ...
      row
        ...
        .attr("width", 18)
        .attr("height", 18)
        .attr("fill", colorScale(job) as string);
      row
        .append("text")
        .attr("x", 24)
        .attr("y", 14)
  ...
  return (
    <Page>
      ...
      <div className="max-w-8xl mx-auto px-4 py-16">
        <SectionCard title="名人特徵地圖 (t-SNE 互動圖)">
